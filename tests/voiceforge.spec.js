...
test.describe("VoiceForge Speech Composer", () => {
  ...
  test("should fallback to browser voice and save to history on successful speech", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator("h1", { hasText: "VoiceForge" }).first(),
    ).toBeVisible();
  ...
  test("should reject empty message and not save to history", async ({
    page,
  }) => {
    await page.goto("/");
