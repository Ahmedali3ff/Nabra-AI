    ...
    async function calculateStats() {
      try {
        ...
        transcripts.forEach((t) => {
          ...
          if (t.language_code) {
            langCounts[t.language_code] =
              (langCounts[t.language_code] || 0) + 1;
          }
        });
    ...
    <div className="space-y-6">
      <header className="rounded-lg border border-ink/10 bg-white p-5 shadow-soft dark:border-border dark:bg-surface">
        ...
        <p className="mt-1 text-sm text-ink/65 dark:text-muted">
          Real-time metrics, costs, and statistics gathered from your local
      ...
      <div className="grid gap-5 sm:grid-cols-3">
        <div className="rounded-lg border border-ink/10 bg-white p-5 shadow-soft dark:border-border dark:bg-surface flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-semibold text-ink/50 dark:text-muted uppercase tracking-wider">
              Total Words Spoken
            </p>
        ...
        <div className="rounded-lg border border-ink/10 bg-white p-5 shadow-soft dark:border-border dark:bg-surface flex items-center gap-4">
          ...
          <div>
            ...
            <h3 className="text-2xl font-bold mt-1 text-moss dark:text-glow">
              ${stats.estimatedCost.toFixed(4)}
            </h3>
            <p className="text-xs text-ink/40 dark:text-neutral-400">
              Calculated locally per character
            </p>
          </div>
        </div>
        ...
        <div className="rounded-lg border border-ink/10 bg-white p-5 shadow-soft dark:border-border dark:bg-surface flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-ink/40 dark:text-neutral-400">
              Avg latency: ~{stats.avgLatencyMs}ms / clip
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <div className="rounded-lg border border-ink/10 bg-white p-5 shadow-soft dark:border-border dark:bg-surface">
          ...
          {stats.voiceRankings.length === 0 ? (
            <p className="text-sm text-ink/50 dark:text-muted py-4 text-center">
              No voice metrics recorded yet.
            </p>
          ) : (
                ...
                return (
                  <div key={vr.id}>
                    <div className="flex justify-between text-sm mb-1 font-semibold">
                      <span>ID: {vr.id}</span>
                      <span className="text-ink/60 dark:text-muted">
                        {vr.count} calls
                      </span>
                    </div>
