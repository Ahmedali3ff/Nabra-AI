...
export default function Library() {
  ...
  const handleAddVoiceToCol = async (colId, voiceId) => {
    const col = (Array.isArray(collections) ? collections : []).find(
      (c) => c.id === colId,
    );
    if (!col) return;
  ...
  const handleLoadOnnx = () => {
    ...
    setTimeout(() => {
      ...
      setOnnxLoaded(true);
      setStatusMsg(
        "ONNX Web Runtime: Tiny-vocoder model weights compiled successfully!",
      );
    ...
    <div className="space-y-6">
      <header className="rounded-lg border border-ink/10 bg-white p-5 shadow-soft dark:border-border dark:bg-surface">
        <h2 className="text-2xl font-bold dark:text-neutral-100">
          Voice Library &amp; AI inference
        </h2>
        <p className="mt-1 text-sm text-ink/65 dark:text-muted">
          Organize cloned voice profiles into collections folders and manage
          local browser weights.
        </p>
      </header>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <div className="rounded-lg border border-ink/10 bg-white p-5 shadow-soft dark:border-border dark:bg-surface space-y-4">
          ...
          <form onSubmit={handleCreateCollection} className="flex gap-2">
            ...
            <button
              type="submit"
              className="rounded-md bg-moss px-4 py-1.5 text-sm font-bold text-white hover:bg-moss/90 dark:bg-glow dark:text-black"
            >
              Create Folder
            </button>
          </form>
          ...
          <div className="space-y-4 mt-4">
            {safeCollections.length === 0 ? (
              <p className="text-sm text-ink/50 dark:text-muted py-2">
                No voice collections folders created yet.
              </p>
            ) : (
              safeCollections.map((col) => (
                <div
                  key={col.id}
                  className="rounded-md border border-ink/10 bg-cloud p-4 dark:border-border dark:bg-black space-y-3"
                >
                  <div className="flex justify-between items-center">
                    <span className="font-bold text-sm text-ink dark:text-neutral-200">
                      {col.name}
                    </span>
                    <button
                  ...
                  <div className="space-y-1">
                    {!Array.isArray(col.voiceIds) ||
                    col.voiceIds.length === 0 ? (
                      <p className="text-xs text-ink/40 dark:text-neutral-500 italic">
                        No voice profiles added yet.
                      </p>
                    ) : (
                      col.voiceIds.map((vId) => {
                        ...
                        return (
                          <div
                            key={vId}
                            className="flex justify-between items-center text-xs bg-white border border-ink/5 rounded px-2 py-1 dark:bg-surface dark:border-border"
                          >
                            <span>{profile?.name || vId}</span>
                            <button
                              onClick={() =>
                                handleRemoveVoiceFromCol(col.id, vId)
                              }
        ...
        <div className="rounded-lg border border-ink/10 bg-white p-5 shadow-soft dark:border-border dark:bg-surface space-y-4">
          ...
          <p className="text-sm text-ink/65 dark:text-muted leading-relaxed">
            Expose local browser-based neural speech synthesis weights.
            Accelerate speech tasks without calling the network.
          </p>

          <div className="rounded-md border border-ink/10 bg-cloud p-4 dark:border-border dark:bg-black text-center space-y-2">
            <span className="text-xs font-semibold text-ink/50 dark:text-neutral-400 uppercase tracking-widest block">
              Status
            </span>
            <span
              className={`text-base font-bold block ${onnxLoaded ? "text-moss dark:text-glow" : "text-ink/60 dark:text-muted"}`}
            >
          ...
          >
            {onnxLoading
              ? "Loading model weights (12MB)..."
              : onnxLoaded
                ? "Weights cached locally"
                : "Load Local tiny-vocoder model"}
          </button>
        </div>
      </div>
    </div>
