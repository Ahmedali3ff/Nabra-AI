...
test("Authentication flow tests", async (t) => {
  ...
  await t.test(
    "Register: fails when missing username or password",
    async () => {
      const req = createRequest({
      ...
      assert.equal(
        res.jsonBody.error,
        "Username and password must be valid strings",
      );
    },
  );

  await t.test(
    "Login: successfully logs in with correct credentials",
    async () => {
      const req = createRequest({
        body: { username: testUsername, password: testPassword },
      });
  ...
  await t.test(
    "Refresh Token: successfully issues a new access token and rotated refresh token",
    async () => {
      const req = createRequest({
      ...
      assert.ok(
        res.jsonBody.refreshToken,
        "Should return a new rotated refresh token",
      );
      assert.notEqual(
        res.jsonBody.refreshToken,
        savedRefreshToken,
  ...
  await t.test(
    "Auth Middleware: fails with invalid/malformed token",
    async () => {
      const req = createRequest({
