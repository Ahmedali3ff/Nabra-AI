...
router.get("/voices", async (req, res, next) => {
  try {
    ...
    const voices = await db.all(
      "SELECT voice_id, name, owner_token, created_at FROM voices",
    );
    res.json(voices);
  } catch (err) {
    next(err);
...
router.get("/voices/:id", async (req, res, next) => {
  try {
    ...
    res.json({
      ...
      audio_base64: voice.audio_blob
        ? voice.audio_blob.toString("base64")
        : null,
...
router.get("/speech-history", async (req, res, next) => {
  try {
    ...
    const rows = await db.all(
      "SELECT * FROM speech_history ORDER BY timestamp DESC",
    );
    res.json(rows);
  } catch (err) {
    next(err);
